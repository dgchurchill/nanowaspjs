...
CODE
      ...
      {
	let bytetemp = readbyte( ${register}R$increment );
	$opcode(bytetemp);
      }
CODE
	}
...
CODE
 ...
 print << "CODE";
      z80.tstates += 2;
      wordtemp = (${arg}R $modifier 1) & 0xffff;
...
sub ini_ind ($) {
    ...
    print << "CODE";
      {
	var initemp = z80.readport( BCR );
	z80.tstates += 2; contend_io( BCR, 3 ); contend( HLR, 3 );
...
sub otir_otdr ($) {
	...
	z80.tstates++; contend( HLR, 4 );
	B = (B-1)&0xff;
	var hltemp = (HLR $modifier 1) & 0xffff; H = hltemp >> 8; L = hltemp & 0xff;
	/* This does happen first, despite what the specs say */
	z80.writeport(BCR,outitemp);
...
sub outi_outd ($) {
	...
	z80.tstates++; contend( HLR, 4 ); contend_io( BCR, 3 );
	var hltemp = (HLR $modifier 1) & 0xffff; H = hltemp >> 8; L = hltemp & 0xff;
	z80.writeport(BCR,outitemp);
	F = (outitemp & 0x80 ? FLAG_N : 0 ) | sz53_table[B];
	/* C,H and P/V flags not implemented */
...
# }
    ...
    if( length $register == 1 ) {
        print << "BIT";
        F = ( F & FLAG_C ) | FLAG_H | ( $register & ( FLAG_3 | FLAG_5 ) );
        if( ! ( ($register) & ( 0x01 << ($bit) ) ) ) F |= FLAG_P | FLAG_Z;
BIT
        if ( $bit == 7 ) {
			print "        if( ($register) & 0x80 ) F |= FLAG_S\n";
		}
    } elsif( $register eq '(REGISTER+dd)' ) {
        print << "BIT";
        z80.tstates += 5;
        let bytetemp = readbyte( tempaddr );
		F = ( F & FLAG_C ) | FLAG_H | ( ( tempaddr >> 8 ) & ( FLAG_3 | FLAG_5 ) );
		if( ! ( (bytetemp) & ( 0x01 << ($bit) ) ) ) F |= FLAG_P | FLAG_Z;
BIT
        if ( $bit == 7 ) {
			print "        if( (bytetemp) & 0x80 ) F |= FLAG_S;\n";
		}
    } else {
        print << "BIT";
        let bytetemp = readbyte( HLR );
        contend( HLR, 4 );
		F = ( F & FLAG_C ) | FLAG_H | ( bytetemp & ( FLAG_3 | FLAG_5 ) );
		if( ! ( (bytetemp) & ( 0x01 << ($bit) ) ) ) F |= FLAG_P | FLAG_Z;
BIT
        if ( $bit == 7 ) {
			print "        if( (bytetemp) & 0x80 ) F |= FLAG_S;\n";
...
sub opcode_shift (@) {
	...
	contend( PC, 3 );
	tempaddr =
	    REGISTER + sign_extend(z80.readbyte_internal( PC++ ));
	PC &= 0xffff;
	contend( PC, 4 );
	opcode3 = z80.readbyte_internal( PC++ );
	PC &= 0xffff;
...
shift
	...
	var opcode2;
	contend( PC, 4 );
	opcode2 = z80.readbyte_internal( PC++ );
	PC &= 0xffff;
	R = (R+1) & 0x7f;
#ifdef HAVE_ENOUGH_MEMORY
	switch(opcode2) {
